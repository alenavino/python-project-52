...
class UserViewsTest(UserTestCase):
    ...
    def test_update_user(self):
        response = self.client.get(reverse_lazy('user_update', kwargs={
            'pk': self.user1.id
        }))
        self.assertEqual(response.status_code, 302)
        self.assertRedirects(response, reverse_lazy('login'))
        ...
        response = self.client.post(reverse_lazy('user_update', kwargs={
            'pk': self.user1.id
        }), data={
            'first_name': 'Garry',
            'last_name': 'Potter',
    ...
    def test_delete_user(self):
        response = self.client.get(reverse_lazy('user_delete', kwargs={
            'pk': self.user1.id
        }))
        self.assertEqual(response.status_code, 302)
        self.assertRedirects(response, reverse_lazy('login'))
        ...
        response = self.client.post(reverse_lazy('user_delete', kwargs={
            'pk': self.user1.id
        }))
        self.assertEqual(response.status_code, 302)
        self.assertRedirects(response, reverse_lazy('users'))
